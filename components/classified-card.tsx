    ...
    <Card className="bg-card border-border hover:border-primary/50 transition-colors">
      <CardContent className="p-5">
        <div className="flex items-start justify-between gap-3 mb-3">
          <Badge variant="outline" className={classifiedTypeColors[classified.classified_type]}>
            {classifiedTypeLabels[classified.classified_type]}
          </Badge>
          <span className="text-xs text-muted-foreground">
            {formatDistanceToNow(new Date(classified.created_at), { addSuffix: true, locale: nl })}
          </span>
        </div>

        <h3 className="text-lg font-semibold mb-2 text-balance">{classified.title}</h3>
