    ...
    <Card className="bg-card border-border hover:border-primary/50 transition-colors">
      <CardContent className="p-4">
        ...
        <div className="flex items-center gap-2 text-sm mb-3">
          {isRetirement ? (
            ...
            <>
              {transfer.from_club ? (
                <>
                  <span className="text-muted-foreground">{transfer.from_club}</span>
                  <ArrowRight className="h-4 w-4 text-primary flex-shrink-0" />
                </>
              ) : (
                <span className="text-muted-foreground italic">Nieuw</span>
              )}
              <span className="text-foreground font-medium">{transfer.to_club}</span>
            </>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span>{format(new Date(transfer.transfer_date), "d MMMM yyyy", { locale: nl })}</span>
        </div>
      </CardContent>
    </Card>
  )
